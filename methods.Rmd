...
- weekdays, [Italian holiday](https://en.wikipedia.org/wiki/Public_holidays_in_Italy) indicators, months, and years as predictor variables in the regression component
  - because we did not have much data on Google-referred traffic, that model only included weekdays as regressors
...
The final model of Google-referred traffic $y_t$ (measured as millions of pageviews) is ARMA(3,2) with overall mean $\mu$, linear trend $\beta_0$, Normally-distributed noise $\epsilon_t$ (having standard deviation $\sigma$), regressors $\mathbf{x}$ with coefficients $\mathbf{\beta}$, and a Gompertz model of change-due-to-intervention $z_t$ (Eq. 1):

\begin{align*}
y_t & = \mu + \sum_{p = 1}^3 \phi_p y_{t - p} + \sum_{q = 1}^2 \theta_q \epsilon_{t - q} + \beta_0 t + \sum_{k = 1}^6 \beta_k x_{t,k} + z_t + \epsilon_t;\\
z_t & = \delta_0 \mathrm{e}^{-d \mathrm{e}^{-\lambda (t - T)}} I_t,
\end{align*}
...
\begin{align*}
  ...
  \sigma, \tau & \sim \text{Cauchy}(0, 5),~\sigma > 0;\\
  \beta_0, \beta_k & \sim \mathcal{N}(0, 10),~k = 0, \ldots, 6;\\
